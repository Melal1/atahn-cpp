...
Salawat is a lightweight and blazing-fast C++ utility for fetching and displaying Islamic prayer times. It leverages the Al-Adhan API and provides a simple command-line interface, as well as a specialized daemon-like mode for system status bars.

## Features

- **Accurate Prayer Times**: Fetches timings for Fajr, Dhuhr, Asr, Maghrib, and Isha via the [Al-Adhan API](https://aladhan.com/api).
- **Efficient Caching**: Limits unnecessary API calls by caching the current day's prayer timings locally at `/tmp/salawat_cache.json`.
...
Running `salawat` directly will load the timings for the day and print them to the standard output:
...
Bar mode allows `salawat` to run as a continuous process, writing the current status (such as a countdown to the next prayer) to a shared memory file. 
