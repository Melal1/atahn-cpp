...
public:
  ...
  constexpr Time(uint16_t h, uint16_t m, uint16_t s = 0) noexcept : hour(h), min(m), sec(s)
  {
  }
...
public:
  ...
  {
    std::tm tm_now = get_local_time();
    _tp = std::chrono::sys_days{std::chrono::year{tm_now.tm_year + 1900} / (tm_now.tm_mon + 1) / tm_now.tm_mday};
  }
